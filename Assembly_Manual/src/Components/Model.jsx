...
/*     THREE.BufferGeometry.prototype.computeBoundsTree = computeBoundsTree;
    ...
    const preparingStepArray = [ //shown grouped
        ...
        [
            "10_Prepare_Back_Separator_Panel"
        ],
        [
            "141_Prepare_Side_Electronic_Plate_1",
            "142_Prepare_Side_Electronic_Plate_2"
        ],
        [
            "171_Prepare_Back_Electronic_Plate_1",
        ...
        [
            "441_Prepare_CO2_Head",
            "442_Fix_Dovetail_Pin_on_Cover",
            "443_Fix_the_Head_Plate"
        ],
        [
    ...
    let exceptionPreparingTempArray = []
            ...
            for (let m = subPrepArray.length - 1; m >= 0; m--) { // count down the elements from subArray
                if (stepName[n] === subPrepArray[m]) { //if the current step is the same as in the subPrepArray, retrieves the model and adds to the preparringTemArray
                    ...
                    if (stepName[n + 1] === "27_Fix_Z_ball_screw_on_Z_front_plate") {
                        let exceptionPreparingStepModel = modelAux.getObjectByName("25_Prepare_Z_ball_screw", true)
                        preparingTempArray.push(exceptionPreparingStepModel)
                    }
                }
            }
    ...
    const preparingStepChange = useCallback(() => {
        ...
        //     });
        preparingTempArray.forEach(obj => {
            obj.visible = true;
            let clonedObj = obj.clone();
            groupVisibleObjPrep.add(clonedObj)
        });
    ...
    const mainMachineBuildStepChange = useCallback(() => {
        if (stepName[stepCount] === "29_Fix_middle_plate_on_X-axis") {
            let excludeArray = [
                "25_Prepare_Z_ball_screw",
                "26_Prepare_Z_front_plate",
                "27_Fix_Z_ball_screw_on_Z_front_plate"
            ]
        ...
        for (let i = stepCount - 1; i >= 0; i--) {
            ...
            //If it is listed in the exception Array, show mode
        }
        let excludeArray = []
        if (stepName[stepCount] === "29_Fix_middle_plate_on_X-axis") {
            excludeArray = [
